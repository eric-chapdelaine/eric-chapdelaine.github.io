...
Which tree is better? Obviously the left because the height is $\log_2 (n)$ if `n` is the number of nodes. Whereas the tree on the right is basically a list.
...
We now want to use an array-based structure instead of a tree.
