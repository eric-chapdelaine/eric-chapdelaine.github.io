...
Suppose $f(x)$ is continuous and $n$-times differentiable function at $a$. Then the polynomial $T _{f} ^{n} \left( x ; a \right) = \sum_{k=0}^{n} \frac{f^{(k)}(a)}{k!}\left( x-a \right)^{k} = \frac{f(a)}{0!} + \frac{f'(a)}{1!}(x-a) + \frac{f''(a)}{2!}(x-a)^2 = \frac{f'''(a)}{3!} (x-a)^3 + ... + \frac{f^{(n)}(a)}{n!} (x-a)^n$
...
$T _{f} ^{n} (x) = \sum_{k=0}^{n} \frac{f^{(k)}(x)}{k!} = 1+x+\frac{x^2}{2!}+\frac{x^3}{3!} + ... + \frac{x^{n}}{n!}$ 
